...
def validate_form():
    global valid, submitted_date, submitted_time, event_date, event_time, num_holidays, form_error
    format_string = "%I:%M %p"
    logging.debug("validate_form called with: submitted_date=%s submitted_time=%s event_date=%s event_time=%s num_holidays=%s",
                  submitted_date, submitted_time, event_date, event_time, num_holidays)
    ...
    if (not submitted_date or not submitted_time
        ...
        logging.warning("Validation failed: negative holidays - %s", num_holidays)
        return
    else:
...
def is_late():
    ...
    global submitted_date, submitted_time, event_date, event_time, num_holidays
    ...
    holidays = int(num_holidays) if num_holidays else 0
    ...
    st.write(f"Event at: {event_dt.strftime('%m/%d %I:%M %p')}")
